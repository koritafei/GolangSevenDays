...
func newService(rvcr interface{}) *service{
	s := new(service)
	s.rvcr = reflect.ValueOf(rvcr)
	s.name = reflect.Indirect(s.rvcr).Type().Name()
	s.typ = reflect.TypeOf(rvcr)
...
func (s *service) registerMethods() {
	...
	for i:=0;i<s.typ.NumMethod();i++ {
		...
		mType := method.Type
		if 3 != mType.NumIn() || 1 != mType.NumOut() {
			continue
		}
...
func (service *Service) call(m *methodType,argv, replyv reflect.Value) error {
	...
	f := m.method.Func
	returnValues := f.Call([]reflect.Value{s.rvcr, argv, replyv})
